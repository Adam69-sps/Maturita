import tkinter
import random
canvas = tkinter.Canvas(width=900,height=300,)
canvas.pack()
...
def vyhra():
    canvas.delete("all")
    canvas.create_text(350,20, font="Arial 20",fill="blue", text="Gratulujem, označil si puknutý tanier!")
...
def viacnasobne():
    ...
    for i in range(len(taniere)):
        if taniere[i] > 1:
            oznacene += pismeno(i)
        canvas.create_text(350,80, text="Viackrát si klikol na taniere: " + oznacene,font="Arial 20",fill="red")

def klik(sur):
    global koniec
    if sur.x < 10*70 and not koniec:
        tanier = sur.x // 70
        taniere[tanier] += 1
        if tanier == puknuty:
            koniec = True
